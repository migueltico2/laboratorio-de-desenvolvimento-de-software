		...
		<v-dialog v-model="detailsDialog" max-width="500px">
			<v-card>
				...
				<v-card-actions>
					...
					<v-btn color="success" variant="text" @click="redeemAdvantage" :loading="redeemLoading"
						>Resgatar</v-btn
					>
				</v-card-actions>
			</v-card>
...
const { listAdvantages, createAdvantage, buyAdvantage, findAccount, listAdvantagesForStudent } = useFetchs();
const { user } = useAuth();
...
const redeemAdvantage = async () => {
	try {
		...
		if (error.response.data.message === 'Not enough coins') toast.error('Você não possui saldo suficiente!');
...
const fetchAdvantages = async () => {
	try {
		if (permissions.value === 'student') {
			const advantagesData = await listAdvantagesForStudent(user.students?.institution?.id);
...
onMounted(async () => {
	if (user.type === 'student') account.value = await findAccount(user.type, user.students?.id);
